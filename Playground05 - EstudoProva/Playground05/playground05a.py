...
def main():
    ...
    def aproximou():
        global timer4
        if timer4 == None:
            timer4 = Timer(3.0, toca_campainha)
            timer4.start()
            print("Aproximou: Iniciando timer")
    ...
    # CRIE UM TIMER RECORRENTE QUE FIQUE MOSTRANDO E
    #APAGANDO A MENSAGEM "ALERTA!" NO LCD A CADA 1 SEGUNDO.
    
    # DICA: USE UMA OUTRA VARIÁVEL GLOBAL BOOLEANA
    #PARA SABER SE É PARA ESCREVER OU APAGAR O LCD.
    global write
    write = True
    ...
    banco = cliente["playground05"]
    colecao = banco["sensor_luz"]
    
    def salva_sensor_luz():
        print("salvando dados do sensor de luz no mongo db")
        agora = datetime.now()
        dados = {"datetime": agora, "sensor_value": sensor_de_luz.value}
        colecao.insert_one(dados)
        timer10 = Timer(6, salva_sensor_luz)
        timer10.start()
    ...
    def envia_msg():
        dados = {"chat_id": id_da_conversa, "text": "Sensor detectou movimento!"}
    ...
    def envia_msg_telegram():
        ...
        if(envia):
            ...
            envia = False   
        elif((datetime.now() - ultimo_envio).total_seconds() >= 30):
            envia_msg()
            ultimo_envio = datetime.now()         
    
    sensor_de_movimento.when_motion = envia_msg_telegram
